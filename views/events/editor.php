...
<form method="post" enctype="multipart/form-data" class="narrow-form panel">
    ...
    <div class="form-grid">
        ...
        <section class="event-background-field span-2">
            <div>
                <strong>Tło ekranu uczestników</strong>
                <p>Zdjęcie będzie używane w trybach „Tło” i „Tekst”. Najlepiej sprawdzi się format 16:9.</p>
                <label>Wybierz JPEG, PNG lub WebP (maks. 12 MB)<input type="file" name="background_image" accept="image/jpeg,image/png,image/webp"></label>
                <?php if (!empty($event['background_image'])): ?><label class="clear-background"><input type="checkbox" name="clear_background" value="1"> Usuń obecne tło</label><?php endif; ?>
            </div>
            <?php if (!empty($event['background_image']) && !empty($event['public_token'])): ?>
                <img src="<?= e(url('event-background', ['token' => $event['public_token'], 'v' => $event['background_image']])) ?>" alt="Obecne tło wydarzenia">
            <?php else: ?>
                <div class="background-placeholder">Brak zdjęcia<br><small>zostanie użyte ciemne tło</small></div>
            <?php endif; ?>
        </section>
